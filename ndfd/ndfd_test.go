...
func TestFetchAndDecode(t *testing.T) {
	...
	resp.Body.Close()
	dwmlGlobal, err = Unmarshal(body)

	if err != nil {
		t.Errorf("%s", err)
	}

	fmt.Println(dwmlGlobal)

}

func TestGenerateTimeSpanCollectionMap(t *testing.T) {
	tsMap, err := generateTimeSpanCollectionMap(dwmlGlobal)
